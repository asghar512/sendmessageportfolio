    ...
    try {
        ...
        // }
            ...
            if (findUser) {
                // const verificationCode = Math.floor(100000 + Math.random() * 900000);
                const verificationCode = Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000
                const updateUser = await User.findOneAndUpdate({ _id: findUser._id }, { user_is_forgot: 1, verification_code: verificationCode, is_verified:0 }, { new: true })
                if (updateUser) {
...
const sendMessage = async(req,res,next)=>{
    try{
    const {name,email,subject,user_dtls} = req.body
    const data = {
        name:name,
        email:email,
        subject:subject,
    ...
    const sendmsg = await messageModel.create(data)
    return res.status(200).send({
        status: 1,
